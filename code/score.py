...
from code.const import COLOR_TURBOBLUE, COLOR_TURBOGREEN, COLOR_FLAME, FONT_OPTION_SIZE, FONT_TITLE_SIZE, MENU_OPTION, WIN_HEIGHT, WIN_SCORE_POS, WIN_WIDTH
...
class Score:
    ...
    def __init__(self, window):
        self.window = window
        self.surf = pygame.image.load('./assets/ScoreBg.png').convert_alpha() #load the background image
        self.rect = self.surf.get_rect(left=0, top=0)  #get the rectangle of the image setting the position (0,0 is default)
        


    def save(self, game_mode: str, player_score: list[int]):
        pygame.mixer_music.load('./assets/Sound_Score.mp3')
        ...
        while True:
            ...
            if updated or namePlay != prev_namePlay or text != prev_text:
                ...
                self.score_text(30, namePlay, COLOR_FLAME, WIN_SCORE_POS['Name'] )
                pygame.display.flip()
