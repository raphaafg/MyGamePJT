    ...
    def run(self):
        ...
        ##----- selecting menu option -----##
            if menu_return in [MENU_OPTION[0], MENU_OPTION[1], MENU_OPTION[2]]: #if player selects to play 1p. 2p coop or 2p vs, starts the level 1
                ...
                level = Level(self.window, 'MAP01', menu_return, player_score) #create a new Level object with the window and level name
                level_return = level.run(player_score)
                if level_return:
                    level = Level(self.window, 'MAP02', menu_return, player_score) #create a new Level object with the window and level name (the second level)
                    level_return = level.run(player_score)
                    if level_return:
